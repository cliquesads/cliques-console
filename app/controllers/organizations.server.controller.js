    ...
    return {
        ...
        create: function(req, res) {
            ...
            organization.save(function (err, org) {
                ...
                // also check for accessLink, aka "Invite URL". If one is present, means
                // we need to deactivate it and deal with any delegated entities (publishers/Advertisers)
                if (org.accessLink){
                    AccessLink.findById(org.accessLink, function(err, accessLink){
                        if (err) return console.error('ERROR occurred when finding accessLink field for org: ' + err);
                        accessLink.expired = true;
                        accessLink.save(function(err){
                            if (err) console.error('ERROR occurred when saving accessLink: ' + err);
                            // try to delegate any Advertisers passed in delegatedAdvertiser field
                            if (accessLink.orgType === 'advertiser' && accessLink.delegatedAdvertiser){
                                advertiserModels.Advertiser.findByIdAndUpdate(
                                    accessLink.delegatedAdvertiser,
                                    { organization: org._id },
                                    function(err, advertiser){
                                       if (err) return console.error('ERROR occurred when updating advertiser: '+ err);
                                ...
                                publisherModels.Publisher.findByIdAndUpdate(
                                    ...
                                    function(err, publisher){
                                        if (err) return console.error('ERROR occurred when updating publisher: '+ err);
                                        console.info(`Successfully delegated ownership of publisher ${publisher.id} to organization ${org.id}`);
                                    });
                            }
                        });
                    });
